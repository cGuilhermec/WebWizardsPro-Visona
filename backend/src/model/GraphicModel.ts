...
const editor_Graph_Area_Vs_Status = async (cidade: string, name: string) => {
  ...
  const editor_Graph_Area_Vs_Status = await client.query(
    `select ROUND(CAST(SUM(area_km2) AS numeric),2) as total_km2, status from ${cidade} WHERE atribuicao = $1 GROUP BY status;`,
    [name]
  );
...
const editor_Graph_Bar_Status_Vs_Analista = async (
  cidade: string,
  name: string
) => {
  const client = await createConection();
...
const editor_Correcoes = async (
  cidadeApontamento: string,
